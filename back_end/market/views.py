...
class CartAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if requester.id == user.id or requester.role == "менеджер":
            ...
            cart = Cart.objects.filter(client=user, product=prod).exclude(
                order__isnull=False)
            if cart:
                cart[0].number = cart[0].number + number
                cart[0].save()
            else:
                Cart.objects.create(
...
class Order_printAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if requester.id == order.client.id or requester.role == "менеджер":
